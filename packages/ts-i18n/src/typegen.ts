...
export async function generateTypesFromModule(modulePath: string, outFile: string, _exportedName?: string): Promise<void> {
  const moduleSpecifier = normalizeModuleSpecifier(modulePath, outFile)
  const content = `// Auto-generated by ts-i18n. Do not edit.
import type { DotPaths, ParamsForKey, TranslatorFor } from 'ts-i18n'
import * as Mod from ${quoteModuleSpecifier(moduleSpecifier)}
...
function normalizeModuleSpecifier(modulePath: string, outFile: string): string {
  if (!isAbsolute(modulePath))
    return modulePath

  let specifier = relative(dirname(outFile), modulePath).split(sep).join('/')
